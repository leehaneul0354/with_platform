...
- **구현된 기능 목록**
  ...
  - **홈 화면 무한 로딩 탈출 및 0원 노출 차단:** (1) `MainScreen` — 홈 콘텐츠에 `KeyedSubtree(key: ValueKey(_isStreamTab0Ready))`로 `_isStreamTab0Ready` 변경 시 강제 리빌드, 워터폴 시작 시 `🚩 [LOG] 워터폴 로딩 시작: 홈 탭` 로그. (2) 메인 홈 후원 영역 — `HopeMessageCard`(정적 희망 메시지) 사용, Firestore/로딩 없음. (3) `ApprovedPostsFeedSliver` — 3초 타임아웃 시 "데이터를 불러올 수 없습니다. 다시 시도해주세요" + [새로고침], 로딩 중 스켈레톤 카드 2개 표시, 에러 시 로그 및 따뜻한 안내. (4) `ShimmerPlaceholder` — 0원 대체용 회색 애니메이션 플레이스홀더(opacity 0.35~0.65 반복).
  ...
  - **상세 페이지 Firestore 스트림 충돌 수정(b815, ca9) 및 ca9 Assertion 박멸:** (1) `PostDetailScreen` — 게시글 본문은 기존처럼 `get()` 단발 조회 + State에 보관, 좋아요/카운트는 `StreamBuilder`·`snapshots()`를 완전히 제거하고 `FutureBuilder<_PostLikeInfo>` + `likes.get()` 단발 조회로 변경. Firestore 호출마다 `await Future.delayed(const Duration(milliseconds: 100))`를 선행해 이전 페이지 스트림과의 타이밍 충돌 회피. (2) `ThankYouDetailScreen` — 감사편지 상세 좋아요 영역을 `FutureBuilder<_ThankYouLikeInfo>` + `thank_you_posts/{id}/likes.get()` 구조로 변경하고, 관련 투병기록 FutureBuilder도 `get()` 호출 전에 100ms 딜레이를 두어 ca9 가능성을 최소화. (3) `AdminThankYouDetailScreen` — 관리자 전용 감사편지 상세의 좋아요 버튼도 동일한 패턴(`FutureBuilder<_AdminThankYouLikeInfo>` + `get()` + 100ms 딜레이)으로 변경. (4) `CommentSection` — 댓글 리스트는 실시간성이 중요해 기존 `commentsStream` 유지하되, 스트림 인스턴스를 State에 캐시해 중복 구독을 막고 에러 시 "다시 시도" 버튼으로 재구독 가능. (5) `AdminPostDetailScreen` — 어드민 게시물 이미지 URL이 null/빈 문자열일 경우 상세한 디버그 로그(keys 및 raw 값)를 남겨, 데이터 로드 실패 상황을 추적 가능하게 개선.
  - **Firestore ca9 원천 차단 (main.dart):** (1) 웹에서 `Settings`에 `webExperimentalForceLongPolling: true` 추가 — gRPC/WebChannel 대신 HTTP Long Polling 사용으로 스트림 충돌 원천 차단. (2) `persistenceEnabled: false` 유지. (3) 앱 시작 시(다른 Firestore 호출 전) `FirebaseFirestore.instance.clearPersistence()` 1회 호출로 IndexedDB 등 오염 캐시 강제 삭제(try-catch로 실패 시 무시). (4) `StreamBuilder`는 위젯 dispose 시 Flutter가 구독을 자동 취소하므로 별도 cancel 불필요. 메인 홈 후원 영역은 Firestore 미사용 정적 `HopeMessageCard`로 교체됨.
  - **닉네임 동기화(Denormalization 일관성):** 사용자가 닉네임 변경 시 `users` 외에 닉네임이 복사 저장된 모든 문서를 일괄 갱신. `NicknameSyncService`(syncNicknameAcrossCollections): posts(patientName), thank_you_posts(patientName), today_thank_you(patientName), comments(userName)를 WriteBatch(500건 단위)로 업데이트. `AuthRepository.updateUser`에서 닉네임이 실제로 변경된 경우에만 동기화 호출(실패 시 로그만, users 문서는 이미 반영).
  - **메인 화면 후원 섹션 경량화(ca9 제거):** 메인 상단 '후원 진행 상황'에서 금액·진행률·프로그레스 바 및 모든 Firestore 통신(getPlatformStats, FutureBuilder 등) 제거. `HopeMessageCard`로 교체 — "오늘도 따뜻한 마음들이 모여 환아들에게 희망의 빛이 되고 있습니다" 정적 메시지만 표시, 렌더 부하·엔진 충돌 원인 제거. `PlatformStatsCard`는 관리자 등 다른 화면에서만 사용 가능(메인에서는 미사용).
...
| 경로 | 역할 |
|------|------|
| `lib/main.dart` | 앱 진입점. 웹 시 Firestore: persistenceEnabled: false, webExperimentalForceLongPolling: true, clearPersistence() 1회. loadCurrentUser 후 WithApp → MainScreen |
...
| `lib/core/auth/auth_repository.dart` | AuthRepository(싱글톤). getUsers/updateUser(닉네임 변경 시 syncNicknameAcrossCollections 호출), SharedPreferences 저장, `signInWithGoogle()` 구글 소셜 로그인 구현, `updateUserOnboardingInfo()` 온보딩 정보 업데이트 |
| `lib/core/services/nickname_sync_service.dart` | 닉네임 변경 시 플랫폼 전역 일관성 유지. syncNicknameAcrossCollections(userId, newNickname): posts/thank_you_posts/today_thank_you/comments에서 해당 userId 문서의 patientName·userName을 WriteBatch(500건 단위)로 일괄 업데이트 |
| `lib/shared/widgets/hope_message_card.dart` | 메인 화면 상단 정적 희망 메시지 카드. Firestore·금액·진행률 없음(ca9 원인 제거). "오늘도 따뜻한 마음들이 모여 환아들에게 희망의 빛이 되고 있습니다" |
...
| `lib/features/main/main_screen.dart` | MainScreen. 폭포수형 순차 로딩(_phaseFeedReady), 상단 후원 영역 HopeMessageCard(정적 메시지), admin 로그인 시 AdminMainScreen pushReplacement |
...
| `lib/features/post/post_detail_screen.dart` | 승인된 사연 상세. 본문은 Firestore `get()` 단발 조회 + State 보관, 좋아요/카운트는 `FutureBuilder<_PostLikeInfo>` + `likes.get()`(100ms 딜레이)로 구현해 상세 진입 시 ca9 Assertion 방지. 이미지 CachedNetworkImageGs. |
...
| `lib/features/main/thank_you_detail_screen.dart` | 감사편지 상세. 좋아요/카운트 영역을 `FutureBuilder<_ThankYouLikeInfo>` + `thank_you_posts/{id}/likes.get()`(100ms 딜레이)로 구현, 관련 투병기록 FutureBuilder도 get() 호출 전 100ms 대기. |
...
| `lib/features/admin/admin_thank_you_detail_screen.dart` | 관리자 전용 감사 편지 상세 풀스크린. 진입 시 admin 재확인, 하단 [삭제][승인], 좋아요/카운트는 FutureBuilder+get() 구조(100ms 딜레이)로 변경해 스트림 충돌 제거. |
...
| `lib/shared/widgets/comment_section.dart` | 댓글 리스트. commentsStream을 State(_commentsStream)에 캐시, initState/didUpdateWidget에서만 할당, 에러 시 "다시 시도" 버튼. |
...
4. **로그인/회원가입**  
   ...
   - **닉네임 변경:** 프로필 수정 등에서 `AuthRepository.updateUser(updated)` 호출 시, 닉네임이 이전과 다르면 `users` 문서 업데이트 후 `syncNicknameAcrossCollections(userId, newNickname)`로 posts/thank_you_posts/today_thank_you/comments의 복사된 닉네임 필드를 WriteBatch로 일괄 갱신(동기화 실패 시 로그만, users는 이미 반영).
...
*마지막 갱신: 메인 후원 섹션 경량화 — HopeMessageCard 도입(정적 희망 메시지, Firestore 제거). 닉네임 동기화 유지.*
